...
from pymultinode.handshake import ConnectionHandler, client_handshake, WebServer, client_web_connect
import eventlet
...
def test_onine():
    """
    Use the handshakes to start the jobs
    """
    dispatcher = Dispatcher()
    library = ConfigurationLibrary()
    connection_handler = ConnectionHandler( dispatcher, library, PASSWORD)
    ...
    for x in range(2):
        ...
        eventlet.spawn(connection_handler.new_connection, client_socket.makefile('rw'))

    dispatcher1, library1 = client_handshake(worker_client.wait().makefile('rw'), PASSWORD)
    dispatcher2, library2 = client_handshake(controller_client.wait().makefile('rw'), PASSWORD)
